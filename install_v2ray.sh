#!/bin/bash
# v2ray一键安装脚本
# Modify by ifeng<https://ifeng.dynv6.net>
...
if [[ "$?" != "0" ]]; then
    IP=`curl -sL -6 ip.sb`
    V6_PROXY="https://ifeng.dynv6.net"
fi
        ...
        if [[ -z "$answer" ]]; then
            ...
            2)
                ...
                do
                    ...
                    ip=`curl -sL http://ip-api.com/json/${host} | grep -E -o "([0-9]{1,3}[.]){3}[0-9]{1,3}"`
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        $CMD_INSTALL nginx
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " Nginx安装失败，请到 https://ifeng.dynv6.net 反馈"
            exit 1
        fi
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        curl -sL https://get.acme.sh | sh -s email=trotter@126.com
        source ~/.bashrc
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请复制上面的红色文字到 https://ifeng.dynv6.net 反馈"
            exit 1
        }
        CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
        ...
        [[ -f $CERT_FILE && -f $KEY_FILE ]] || {
            colorEcho $RED " 获取证书失败，请到 https://ifeng.dynv6.net 反馈"
            exit 1
        }
    else
...
Documentation=https://ifeng.dynv6.net
...
menu() {
	...
	echo -e "# ${GREEN}维护${PLAIN}: 海峰(ifeng)                                         #"
    echo -e "# ${GREEN}网址${PLAIN}: https://ifeng.dynv6.net                             #"
    echo -e "# ${GREEN}TG群${PLAIN}: https://t.me/+Ju9yo5JhLyswNzg5                      #"
    echo -e "#                                                           #"
    echo -e "#  向${GREEN}网络跳越${PLAIN}致敬！！！                                     #"
    echo -e "#  该脚本原作者为${GREEN}网络跳越${PLAIN}，好像已经停止维护。该脚本默认     #"	
    echo -e "#  支持BBR加速，支持ipv6连接。目前由${GREEN}海峰${PLAIN}修改Bug进行维护。   #"	
    echo -e "#                                                           #"	
    echo "#############################################################"
